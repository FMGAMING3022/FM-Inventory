...
RegisterNetEvent('inventory:server:OpenInventory', function(name, id, other)
	...
	if not ply.state.inv_busy then
		if name and id then
			...
			TriggerClientEvent("FM-Inventory:client:closeinv", id)
	...
	elseif QBCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		...
		elseif QBCore.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
            if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
                ...
                local imageurl = ("https://cfx-nui-FM-Inventory/html/images/%s.png"):format(itemData.name)
...
RegisterNetEvent('FM-Inventory:server:SaveStashItems', function(stashId, items)
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot) then
			if AddItem(target, item.name, amount, false, item.info, item.created) then
				...
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
				TriggerClientEvent('FM-Inventory:client:giveAnim', src)
				TriggerClientEvent('FM-Inventory:client:giveAnim', target)
			else
...
QBCore.Functions.CreateCallback('FM-Inventory:server:GetStashItems', function(source, cb, stashId)
