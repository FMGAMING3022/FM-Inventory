...
function showBlur() {
    $.post('https://FM-Inventory/showBlur');
}
...
//     localStorage.setItem('FM-Inventory-color', color);
// }

// const savedColor = localStorage.getItem('FM-Inventory-color');
...
function FormatAttachmentInfo(data) {
    $.post(
        "https://FM-Inventory/GetWeaponData",
        JSON.stringify({
            weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post(
                "https://FM-Inventory/RemoveAttachment",
                JSON.stringify({
...
$(document).on("click", ".CombineItem", function(e) {
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post(
            "https://FM-Inventory/combineWithAnim",
            JSON.stringify({
        ...
        $.post(
            "https://FM-Inventory/combineItem",
            JSON.stringify({
...
function optionSwitch(
    ...
    $.post(
        "https://FM-Inventory/SetInventoryData",
        JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if (
            ...
            $.post("https://FM-Inventory/PlayDropSound", JSON.stringify({}));
            $.post(
                "https://FM-Inventory/SetInventoryData",
                JSON.stringify({
                    fromInventory: $fromInv.attr("data-inventory"),
            ...
            if (fromData.amount == $toAmount) {
                ...
                if (
                    ...
                    $.post(
                        "https://FM-Inventory/getCombineItem",
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("https://FM-Inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Open = function(data) {
        ...
        totalWeightOther = 0;
        var v = data
        $('.namejs').html('<i class ="fa fa-id-badge"></i> ID:'+v.pid+' | NAME: '+v.pname+' |  '+'<i class="fas fa-wallet"></i>'+' CASH: $'+v.bank)
    ...
    Inventory.Close = function() {
        ...
        $.post("https://FM-Inventory/CloseInventory", JSON.stringify({}));
